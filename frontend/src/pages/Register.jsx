...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to BookBase 🎉');
      navigate('/');
    } catch (err) {
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="card p-8">
          ...
          <p className="text-gray-400 text-sm mb-6">Join thousands of readers</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Full Name</label>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="John Doe" className="input" required />
            </div>
            <div>
